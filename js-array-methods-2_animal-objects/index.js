...
const animalsSortedByWeightReversed = animals.slice().sort((b, a) => {
  if (a.weight < b.weight) {
    return -1;
  } else if (a.name > b.name) {
    return 1;
...
const weightOfAllAnimalsInAfrica = animals
  .filter((animal) => animal.continents.includes("Africa")) // Filter the animals that live in Africa
  .reduce((total, animal) => total + animal.weight, 0); // Calculate the total weight of the filtered animals
